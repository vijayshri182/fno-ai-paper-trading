...
def _equity_curve_from_steps(steps: list[SessionStep]) -> tuple[EquityPoint, ...]:
    return tuple(EquityPoint(time=step.bar.timestamp, equity=step.equity) for step in steps)
...
def report_to_html(report: SessionReport) -> str:
    ...
    counter_rows = kv_rows([
        ...
        ("Trades closed", str(report.trades)),
...
<div class="wrap">
  <header>
    <h1>{title}</h1>
    <p>Generated {escape(generated)} &middot; paper-session operations &middot; {escape(report.source)}</p>
  </header>
