...
| **Phase 7 (post-V1, in progress — evaluation-first)** | AI decision-support foundation (advisory only), deterministic feature engineering, market regime detection, historical strategy evaluation, ~5-year replay capability, the durable experience store, outcome analysis / candidate generation, champion vs challenger evaluation, controlled promotion/rollback via an evidence-gated promotion gate + version registry, and the continuous feedback/learning loop (paper-trade replay → experience capture → hypotheses → comparison → gate → promoted champion feeds the next cycle) are implemented; MA(5,21) is the frozen baseline; regime-aware and AI candidates are researched/validated (historical + out-of-sample, no look-ahead, no leakage) before adoption — see `PROJECT_PLAN.md` §17d/§17e/§17g |
